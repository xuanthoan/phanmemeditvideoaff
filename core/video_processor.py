...
class VideoProcessor:
    ...
    def cut_segments(self, input_video: Path, scenes: list[tuple[float, float]], out_dir: Path) -> list[Path]:
        ...
        if not scenes:
            ...
            run_cmd([self.ffmpeg, "-y", "-hide_banner", "-loglevel", "error", "-i", str(input_video), "-c", "copy", str(out)], self.on_log)
            return [out]
        for idx, (start, end) in enumerate(scenes):
            out = out_dir / f"seg_{idx:03d}.mp4"
            run_cmd([self.ffmpeg, "-y", "-hide_banner", "-loglevel", "error", "-ss", str(start), "-to", str(end), "-i", str(input_video), "-c", "copy", str(out)], self.on_log)
            segments.append(out)
        return segments
    ...
    def concat(self, segments: list[Path], out_path: Path) -> None:
        ...
        run_cmd([self.ffmpeg, "-y", "-hide_banner", "-loglevel", "error", "-f", "concat", "-safe", "0", "-i", str(list_file), "-c", "copy", str(out_path)], self.on_log)
