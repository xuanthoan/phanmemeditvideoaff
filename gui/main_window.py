...
from utils.file_helper import collect_images, collect_videos, ensure_output
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.last_openable_output: Path | None = None
    ...
    def start(self):
        ...
        if self.output_dir:
            self.last_openable_output = self.output_dir
        else:
            self.last_openable_output = ensure_output(videos[0], None)
    ...
    def open_output(self):
        target = self.output_dir or self.last_openable_output
        if not target or not target.exists():
            QMessageBox.warning(self, "Output", "Chưa có thư mục output để mở.")
            return
